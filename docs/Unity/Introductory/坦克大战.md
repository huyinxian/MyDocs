...
![](http://cdn.fantasticmiao.cn/image/post/U3D/TankGame/01.png)

首先就是要把自动渲染关掉，不然的话编辑器会自动给你渲染，很占资源。然后将`Source`切换为`Color`，根据地图的颜色选择棕黄色，这样地图就看起来没那么亮。

![](http://cdn.fantasticmiao.cn/image/post/U3D/TankGame/02.png)
...
![](http://cdn.fantasticmiao.cn/image/post/U3D/TankGame/03.png)
...
![](http://cdn.fantasticmiao.cn/image/post/U3D/TankGame/04.png)
...
![](http://cdn.fantasticmiao.cn/image/post/U3D/TankGame/05.png)
...
![](http://cdn.fantasticmiao.cn/image/post/U3D/TankGame/06.png)
...
![](http://cdn.fantasticmiao.cn/image/post/U3D/TankGame/07.png)
...
![](http://cdn.fantasticmiao.cn/image/post/U3D/TankGame/08.png)
...
![](http://cdn.fantasticmiao.cn/image/post/U3D/TankGame/09.png)
...
![](http://cdn.fantasticmiao.cn/image/post/U3D/TankGame/10.png)

### 为坦克添加爆炸效果

这还不够，我们也得为坦克添加爆炸效果。步骤的话与上面的类似，只不过需要在`TankHealth`中调用爆炸特效。

![](http://cdn.fantasticmiao.cn/image/post/U3D/TankGame/11.png)
...
![](http://cdn.fantasticmiao.cn/image/post/U3D/TankGame/12.png)

为了达成这种效果，可以先将摄像机调整成下面这样：

![](http://cdn.fantasticmiao.cn/image/post/U3D/TankGame/13.png)
...
![](http://cdn.fantasticmiao.cn/image/post/U3D/TankGame/14.png)
...
![](http://cdn.fantasticmiao.cn/image/post/U3D/TankGame/15.png)

修改`Fill`的`Image Type`属性，将其设为`Filled`。你可以在这里调整进度条的颜色和填充方式：

![](http://cdn.fantasticmiao.cn/image/post/U3D/TankGame/16.png)

![](http://cdn.fantasticmiao.cn/image/post/U3D/TankGame/17.png)

### 将血条添加到坦克身上

当前的血条仍然是 2D 形式的，为了让它渲染成 3D，我们需要修改一下它的渲染模式：

![](http://cdn.fantasticmiao.cn/image/post/U3D/TankGame/18.png)

接下来把整个画布添加到`Tank`下，重设一下 UI 的位置，这样血条就会附加在坦克上。当然，你会发现血条的尺寸和旋转角度不对，我们需要调整一下：

![](http://cdn.fantasticmiao.cn/image/post/U3D/TankGame/19.png)
...
![](http://cdn.fantasticmiao.cn/image/post/U3D/TankGame/20.png)
