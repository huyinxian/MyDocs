...
UGUI 中首先要注意的一点就是让 UI 的 z 坐标为 0。如果 UI 的 z 坐标不为 0，那么 Unity 就会认为当前的 UI 具备深度，当成 3D UI 处理，导致很多 DrawCall 无法合并。事实上，我们在制作 UI 元素时，其实并不需要调整 z 值来实现遮挡关系，因此我们理应保证所有的 UI 元素的 z 坐标都为 0。
...
这个问题确实是非常令人头疼的，因为如果你的游戏需求就是要让气泡往旁边偏一点，那么你就没有办法避免图标的重叠。要解决这个问题，基本上就只能够单独把所有红点放到一个 Canvas 下。不过说是这么说，真正去做的话其实是非常麻烦的，因为项目一般的做法都是以 Item 为单位进行管理，强行将 Item 进行拆分只会带来管理上的不便，所以最好还是缩小 Item 的尺寸避免重叠的发生。
