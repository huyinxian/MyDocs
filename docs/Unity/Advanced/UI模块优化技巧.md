...
相比于可以看得到源码的 NGUI，UGUI 内部会自动对 DrawCall 进行调整。UGUI 中并没有像 NGUI 那样设计一个 `Depth` 控制 DrawCall 的合并，它的合并顺序主要取决于 Hierarchy 视图中的节点顺序，排在越下面的元素渲染后就会越靠前。下面将用一个简单的例子来说明 UGUI 的合并规则。
