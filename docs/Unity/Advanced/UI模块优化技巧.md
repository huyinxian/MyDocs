...
在进行 UI 渲染前，会对网格进行更新重建。可以说，绝大部分的 UI 开销都是在这里。**网格更新**指的是 UI 元素本身的属性发生变化（比如 UGUI 中就是修改 `UIVertex` 中的顶点属性），所以需要更新或者重新生成顶点属性。举个例子，UI 元素的颜色是由顶点颜色决定的，因此修改 UI 颜色时就需要对顶点进行更新。对于 UI 元素位置改变这种情况，一般来说是不会造成网格更新的，而是会引起**网格重建**。以 UGUI 为例，一个 `Canvas` 下的所有 UI 会被合并成一个大的 Mesh，单个 UI 元素的位置改变将会引起整个 `Canvas` 的重建。

总而言之，顶点属性的更新以及位置的变化都会引起网格的重建，而完全静态的 `Canvas` 是不需要进行网格重建的。
...
由于 UGUI 每次都会对网格进行重建，所以我们就不需要关注上面提到的问题。UGUI 中的敏感操作是对 UI 元素的 `position` 赋值，因为每次赋值时 UGUI 并不会判断这个值有没有发生变动，它会直接调用 `Canvas.BuildBatch` 进行更新。为了避免无效赋值的情况，我们在每次改变 UI 元素的位置前都需要先判断数值是否发生了改变。
