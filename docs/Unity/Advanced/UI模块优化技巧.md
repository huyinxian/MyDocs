...
网格更新的标志性函数是 `Canvas.SendWillRenderCanvases`，即对 UI 元素进行重绘。在 UGUI 中，UI 元素的更新会被分成两种，第一种是 `RectTransform` 发生了变化（比如修改了 `Size`、`Anchor`、`Pivot` 会对 `UIVertext.position` 产生影响），第二种是渲染元素发生了变化（比如修改了 Image 和 Text 的颜色）。
