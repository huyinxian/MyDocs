...
?> NGUI 的 DrawCall 之所以要比 UGUI 好控制，其实很大程度上取决于 Depth 的计算方式。NGUI 的 Depth 可以手动进行调节，因此很少存在 DrawCall 被打断的情况。UGUI 的显示顺序取决于 Hierarchy 的层级，虽然在制作界面时可以很直观的理清楚 UI 的结构，但也使得我们无法手动调节各个元素的层级，进而导致 DrawCall 经常被打断。
