...
新方案的核心思想是直接将数据存储到 Lua 的数组中，然后将 Lua 的底层内存结构直接暴露给 C# 端，让 C# 不经过 Lua API 直接以 `unsafe` 的方式操作 Lua 内存。这样一来，Lua 端可以直接读写 Lua 数组，C# 端则以 unsafe 方式读写 Lua 内存，从而避免了堆栈交互所带来的消耗。

具体的做法我目前还在研究，等做的差不多会重新写一篇文章来分享。
