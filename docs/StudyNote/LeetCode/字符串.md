...
我们注意到，0 到 m 这一段字符串是 T[i...i+j-1] 的后缀，同时也是 P[0...j-1] 的前缀。由于 T[i...i+j-1] 和 P[0...j-1] 是相等的，并且我们要求最大的 m，那么问题其实就转化成**求已匹配字符串的前缀和后缀的公共前缀子串**。

如何计算这个公共部分呢？我们需要用到**部分匹配表**（Partial Match Table）。对于字符串 `abababca`，它的 PMT 表如下：
...
PMT 中的值指的是从 0 到位置 i 的字符串的前缀集合和后缀集合的公共前缀子串长度。比如对于 `aba` 它的前缀和后缀的公共前缀部分为 `a`，因此它的 PMT 值就为 1。
