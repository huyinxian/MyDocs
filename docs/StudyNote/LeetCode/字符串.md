...
选择的方式很简单，我们需要选择最大的 m。如果选择较小的 m，比如我们直接回退到首位 0，那么很有可能会丢失掉较大 m 的匹配情况。因此，m 要尽可能的大，避免遗漏掉某些匹配情况。
...
先来说说坏字符规则。我们让模板串与主串由后往前比较，当遇到主串中不匹配的字符 a 时，我们记字符 a 在模板串中对应的位置为 s。接下来，我们在模板串中**由后往前**寻找字符 a。如果找了，那么记该位置为 x；如果没找到，那么 x 就为 -1。
