...
求 next 数组可以看成是字符串匹配的过程，也就是以待匹配字符串 P 为主串，以 P 的前缀为目标字符串，返回的是匹配成功的长度。具体的来说，就是从主串的第二位开始（首位的 next 记为 -1）进行匹配运算，任意位置上所能够匹配的最大长度就是该位置的 next 值。这里计算 next 的算法和 KMP 是类似的。
