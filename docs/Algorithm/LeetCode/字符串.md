...
字符串匹配的经典算法为 KMP 算法。暴力检索之所以效率低，是因为当匹配失败时，每次都只会移动到下一个字符进行判断，并且待匹配的字符串也要从头开始匹配，导致时间复杂度为 O(N²)。KMP 算法则是每次都会从匹配失败的位置继续，且待匹配的字符串并不会直接从头开始。具体怎么做请往下看。
...
在暴力检索中，主串会回退到 i+1 位置，而待匹配字符串则会回退到 0 位置。为了加快效率，不让主串回退，那么待匹配字符串应该回退到哪个位置才能保证匹配不出错呢？

首先我们知道，从 i 到 i+j-1 的这一段字符串是已经匹配成功的，也就是说存在 `T[i...i+j-1] = P[0...j-1]`。KMP 算法便是利用已经匹配成功的字符串，使得待匹配字符串回退到的位置能够刚好与主串中已匹配完成的字符串**对齐**。如下图：
...
选择的方式很简单，我们需要选择最大的 m。如果选择较小的 m，那么很有可能会丢失掉较大 m 的匹配情况，而如果选择的是较大的则不会有遗漏。
...
PMT 中的值指的是字符串的前缀集合和后缀集合的最大重复长度。比如对于 `aba` 它的前缀和后缀的最大重复部分为 `a`，因此它的 PMT 值就为 1。
...
求 next 数组可以看成是字符串匹配的过程，也就是以待匹配字符串 P 为主串，以 P 的前缀为目标字符串，返回的是匹配成功的长度。具体的来说，就是从主串的第二位开始（首位的 next 记为 -1）进行匹配运算，每个位置上所能够匹配的最大长度就是该位置上的 next 值。注意，在计算 next 的时候同样要利用之前算好的 next 值，不要总是从头开始匹配：
