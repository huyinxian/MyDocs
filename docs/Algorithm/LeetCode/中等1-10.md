...
在解答这个问题，首先要搞清楚什么是有效的括号组合。对于某一个由括号组成的字符串，每一个左括号必定要有一个右括号与之对应。简单点来说，我们先往字符串插入左括号，然后检测左括号数是否小于右括号数，小于时就在字符串中插入一个右括号。当左右括号都为 0 时，输出这个字符串并返回。
